...
class MySetPasswordForm(SetPasswordForm):
    ...
    new_password2 = forms.CharField(label=_("New password confirmation"),widget=forms.PasswordInput(attrs={'autocomplete': 'email','class':'form-control'}))

class CustomerProfileForm(forms.ModelForm):
    class Meta:
        model = Customer
        fields = ['name','locality','city','state','zipcode']
        widgets = {'name':forms.TextInput(attrs={'class':'form-controls'}), 
                   'locality':forms.TextInput(attrs={'class':'form-controls'}), 
                   'city':forms.TextInput(attrs={'class':'form-controls'}), 
                   'state':forms.Select(attrs={'class':'form-controls'}), 
                   'zipcode':forms.NumberInput(attrs={'class':'form-controls'})}
